...
class MapRenderer:
    ...
    def render(self):
        if config.data.features.seeAllTiles and not self._all_tiles_rendered:
            for x in range(self._area_map.width):
                for y in range(self._area_map.height):
                    ...
                    Game.con.draw_char(x, y, tile.character, tile.dark_colour)
        ...
        for (x, y) in self.visible_tiles:
            tile = self._area_map.tiles[x][y]
            Game.con.draw_char(x, y, tile.character, tile.colour)
        ...
        if Game.draw_bowsight:
            ...
            monster_on_target_tile = [x for x in self._area_map.get_entities_on(x2, y2) if x.has_component(Fighter)]
            for pos in line:
                if pos in self.visible_tiles:
                    if pos == (x2, y2) and monster_on_target_tile:
                        Game.con.draw_char(pos[0], pos[1], 'X', fg=colors.red)
                    else:
        ...
        Game.root.blit(Game.con, 0, 0, self._area_map.width, self._area_map.height, 0, 0)
        ...
        player_fighter = self._player.get_component(Fighter)
